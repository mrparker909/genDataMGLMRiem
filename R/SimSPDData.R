...
#' @return If C=NULL (no confounds), returns a data.frame with 2*n_y columns, and N rows. n_y is dims*(dims/2+1/2), or dims*(dims/2-1/2) depending on whether the main diagonal is included. Otherwise, returns a list with elements Y, X, V, and yx, where Y is the list of observed SPD matrices, X is the data.frame of k confounds (Nxk), V is the list of k SPD covariate matrices, and yx is the data.frame of upper diagonal elements of Y (with each row corresponding to one observation Y[[i]]).
...
#' @param C       a kxN array of confounds (each row is an N-vector corresponding to a set of observations for one confound, each column corresponds to a k-vector of confound observations for the ith individual)
#' @export
genSPDdata <- function(N=500, dims=5, maxDist = 1, SNR=1, includeDiagonal=F, beta=NULL, C=NULL) {
  ...
  if(is.null(C)) {
    ...
    Yp[,,1] = diag(rep(1, times=dims)) # base point is I_dimsxdims
    for(i in 1:k) {
    ...
    return(list(Y=Y, X=C, V=V, y_upper=cbind(y_df)))
  }
}
